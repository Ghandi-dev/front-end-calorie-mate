    ...
    <div className={cn("card min-h-[178px]", { skeleton: isLoading }, { "bg-base-100": !isLoading })}>
      <div className="p-6 flex flex-col w-full">
        <div className="flex items-center justify-between">
          <h3 className="card-title text-content">{t("title")}</h3>
          {!data && (
          ...
          {data ? (
            <Fragment>
              <div className="w-2/4 lg:w-full flex flex-col items-center justify-between gap-2">
                ...
                <h1 className="font-semibold text-sm lg:text-lg text-content text-center">{data?.weight} kg</h1>
              </div>
              <div className="w-2/4 lg:w-full flex flex-col items-center justify-between gap-2">
                <Image src="/images/height.svg" alt="food" width={50} height={50} />
                <h1 className="font-semibold text-sm lg:text-lg text-content text-center">{data?.height} cm</h1>
              </div>
              <div className="w-2/4 lg:w-full flex flex-col items-center justify-between gap-2">
                <Image src="/images/notes.svg" alt="food" width={50} height={50} />
                <h1 className="font-semibold text-sm lg:text-lg text-content text-center">bmr {data?.bmr}</h1>
              </div>
              <div className="w-2/4 lg:w-full flex flex-col items-center justify-between gap-2">
                <Image src="/images/notes.svg" alt="food" width={50} height={50} />
                <h1 className="font-semibold text-sm lg:text-lg text-content text-center">tdee {data?.tdee}</h1>
              </div>
            </Fragment>
          ) : (
